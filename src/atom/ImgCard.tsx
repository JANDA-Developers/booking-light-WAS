import { Bold, DEFAULT_PHOTO, Flex, IJDalignProp, JDalign, JDphotoFrame, JDtypho } from '@janda-com/front';
...
export const ImgCard: React.FC<IImgCardProps> = ({ alignImg, foot, head, children, img, onImgClick: handleImgClick, className, photoProp, ...props }) => {
    ...
    return <JDalign className={classes} {...props}>
        <div ref={ref as any}>
            <Flex column={column} oneone>
                <PhotoFrame ratio={1} src={img || DEFAULT_PHOTO} className="imgCard__photo" onClick={handleImgClick} isBgImg {...photoProp} />
                <Flex column between className="imgCard__contents">
                    <div>
                        {head && <div className="imgCard__head">
                            {head}
                        </div>}
