...
import { InputText, JDbutton, JDcard, JDconfiger, JDcontainer, JDhorizen, Mb, useInput } from "@janda-com/front";
import { FormCreater } from "../../../component/formCreater/FormCreater";
import { BoardDocInput, BoardInput, Fattribute, Fboard, UserRole } from '../../../type/api';
import { useBoardCreate, useBoardDelete, useBoardUpdate } from "../../../hook/useBoard";
import { CardBtn } from '../../../component/btns/ModalBtn';
import { ALLOW_LOGINS } from '../../../type/const';
...
const PermissionItems = ALLOW_LOGINS.map(role => ({
    _id: role,
    name: role
}))

const BasicFields: BaseFiledKey[] = ["contents", "isOpen", "title", "thumb", "summary", "subTitle", "keyWards"]
...
export const BoardConfiger: React.FC<IProp> = ({ board }) => {
    ...
    const [eanbleItems, setEanbleItems] = useCopy<string[]>(board?.fields || []);
    const [readPermission, setReadPermssion] = useCopy<UserRole[]>(board?.readPermission || []);
    const [writePermission, setWritePermssion] = useCopy<UserRole[]>(board?.writePermission || []);
    ...
    return <div>
        <JDcontainer verticalPadding >
            ...
            <JDcard foot={
                <div>
                    <CardBtn thema="primary" onClick={handleBoardCreate} hide={!isCreate} mr>게시판 생성</CardBtn>
                    <CardBtn thema="primary" onClick={handleBoardUpdate} hide={isCreate} mr>게시판 수정</CardBtn>
                    <CardBtn thema="error" onClick={handleDeleteBoard} hide={isCreate} mr>게시판 삭제</CardBtn>
                </div>
            }>
