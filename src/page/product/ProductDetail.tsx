...
import {
    IProductModalInfo,
    ProductCreateModal,
} from "./component/ProductCreateModal";
...
import { ProductCreateTimeCheckModal } from "./component/ProductCreateTimeCheckModal";
...
export const ProductDetail: React.FC<IProp> = ({ item }) => {
    ...
    return (
        <div>
            <JDpageHeader
                title="판매설정"
                desc="판매설정 페이지에서 품목을 어느만큼 언제 판매할지 결정합니다."
            />
            ...
            <JDbutton mb onClick={handleEdit} hide={isCreate} thema="primary">수정하기</JDbutton> */}
                {item && (
                    <ItemCard
                        ...
                        head={
                            <Flex vCenter>
                                <Bold mr="small">상품군</Bold> {item.name}에
                                대한 상품설정
                            </Flex>
                        }
                        mb
                                ...
                                <ProductTable
                                    handleEdit={(product) => {
                                        productCreateModal.openModal({
                                            ...
                                            product: product as FproductBooking,
                                        });
                                    }}
                                    handleDelete={({ _id }) => {
                                        handleDelete(_id);
                                    }}
                                    products={paging.slice as any[]}
                                />
                                <Mb />
                                <Flex center>
                                    <JDpagination
                                        mb
                                        {...DEFAULT_PAGINATION_SETTING}
                                        pageCount={paging.paging.totalPageSize}
