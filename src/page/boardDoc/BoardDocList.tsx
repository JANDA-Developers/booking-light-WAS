import { Bold, Flex, JDbutton, JDcontainer, JDpageHeader, Mb, Small, Thin, WindowSize } from '@janda-com/front';
import React, { useContext, useEffect } from 'react';
import { useHistory } from 'react-router';
import { useBoardDocList } from '../../hook/useBoardDoc';
import { ListInitOptions } from '../../hook/useListQuery';
import { boardDocList, boardDocListVariables, FboardDoc, _BoardFilter, _BoardSort } from '../../type/api';
...
export const BoardDocList: React.FC<IProp> = ({
    ...
    const { params: { boardKey } } = useRouteMatch<IDetailRouteProp>()
    ...
    const hasWritePermission = board?.writePermission.includes(superMe?.role || me!.role);
    ...
    return <div>
        ...
        <JDcontainer verticalPadding size={WindowSize.full}>
            <JDcard mb>
                {items.map(item => <Flex className="boardDocLine" vCenter key={item._id}>
                    <Thin mr="large">{yyyymmddHHmm(item.createdAt)}</Thin>
                    <Bold onClick={handleView(item)} hover mr="large">{item.title || "타이틀 없음"}</Bold>
                    <Small mr>{item.authorName}</Small>
                </Flex>
                )}
            </JDcard>
            <JDbutton hide={!hasWritePermission} mb onClick={handleToWrite} label="글쓰기" />
            <Pagination totalPageCount={pageInfo.totalPageCount} {...paginatorHook} />
        </JDcontainer>
    </div>;
};
