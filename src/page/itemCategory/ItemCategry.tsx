import { Flex, JDalign, JDbutton, JDcontainer, JDpageHeader, JDpreloader, Mb, Mr, SkipUpdate, Tiny, useModal, } from '@janda-com/front';
...
import { useCategoryCreate, useCategoryDelete, useCategoryList, useCategoryOrderUpdate, useCategoryUpdate } from '../../hook/useCategory';
import { useItemList } from '../../hook/useItem';
import { CategoryType, Fcategory, itemList_ItemList_items_ItemBooking, _CategorySort, _IItemFilter, _IItemSort } from '../../type/api';
...
export const ItemCategory: React.FC<IProp> = () => {
    ...
    const { items } = useItemList({
        initialViewCount: 9999, fixingFilter: {
            _storeId__eq: selectedStoreId
        },
    });
    ...
    const [deleteMu] = useCategoryDelete({
        onCompleted: ({ CategoryDelete }) => {
            completeMsg(CategoryDelete, "카테고리 삭제성공", "카테고리 삭제실패");
        }
    });
    ...
    const handleDelete = (item: Fcategory) => () => {
        promptConfirm(item.label, `카테고리를 삭제하실려면 ${item.label}을 정확하게 입력 해주세요.`, () => {
            deleteMu({
                variables: {
                    categoryId: item?._id
                }
    ...
    const unSorted = categories.filter(item => !ordered.find(o => o === item._id)!);
    const sorted = ordered.map(o => categories.find(i => o === i._id)!).filter(v => v);
    ...
    return <div>
        ...
        <JDpageHeader title="상품 카테고리" desc="상품 카테고리를 관리하세요." />
        <JDcontainer verticalPadding>
            <Flex>
                <JDsearchBar searchOps={[{ label: "이름", value: "_id__eq" }]} {...itemListHook} />
                <Mr />
                <JDbutton blink={!isEmpty(ordered)} onClick={handleCategoryOrderUpdate} thema="black" label="순서변경 저장하기" />
            </Flex>
            <Mb />
            <DotButton mb onClick={handleCreate}>생성하기</DotButton>
            <Empty empty={isEmpty(categories)} />
            <SkipUpdate skip={getLoading} >
                <Dragger<Fcategory> onOrder={handleDrop} dir="vertical" handle ulClassName={"formCreater__draggWrap"} idKey="_id" items={merged}
                    ItemRender={(item, index, { dragHandleProps }) =>
                        <JDalign mb {...dragHandleProps}>
                            <CategoryCard items={itemfilterByCat(item)} foot={<div>
                                <CardBtn onClick={handleEdit(item)} mr thema="primary">수정하기</CardBtn>
                                <CardBtn onClick={handleDelete(item)} thema="error" >삭제하기</CardBtn>
                            </div>} category={item} />
                        </JDalign>
                    }>
                </Dragger>
        ...
        <PormptModal key={categoryModalHook.info?.modalKey} modalHook={categoryModalHook} />
    </div>;
};
