...
export const PayRegist: React.FC<IProp> = () => {
    ...
    return <div>
        <JDpageHeader title="결제수단 등록하기" desc="잔다 요금제를 지불할 결제수단을 등록 해주세요." />
        <JDcontainer verticalPadding size={WindowSize.lg}>
            <CardInfoRegister mb />
            {paymethods?.map((paymethod, index) =>
                <CardViewer head="등록된 카드들" foot={<CardBtn thema="error" onClick={handleBillingMethodDelete(paymethod._id)} >등록해제</CardBtn>} billingMethod={paymethod} key={"PayMethod" + index} />
            )}
        </JDcontainer>
    </div>
};
