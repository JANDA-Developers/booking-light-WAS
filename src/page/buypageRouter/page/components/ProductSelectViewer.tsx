...
import { storeFindByCode_StoreFindByCode_items } from "../../../../type/api";
...
export const ProductSelectViewer: React.FC<IProp> = ({
    bookingInputs,
    ...props
}) => {
    const {
    ...
    const bookingInputWithItem: IViewData[] =
        items?.map((item) => ({
            ...item,
            bookingInputs: bookingInputs.filter((bi) => bi.itemId === item._id),
        })) || [];

    const filteredItems = bookingInputWithItem.filter(
        (view) => !isEmpty(view.bookingInputs)
    );

    return (
        <JDcard head={"선택확인"} {...props}>
            {filteredItems.map((itemView, index) => (
                <div key={itemView._id}>
                    <ItemViewBox
                        isLast={isLast(index, filteredItems)}
                        {...itemView}
                        key={itemView._id}
                    />
...
const ItemViewBox: React.FC<ItemViewBoxProp> = ({
    _id,
    thumbNail,
    name,
    bookingInputs,
...
const ProductView = (bi: IBookingInputData) => {
    ...
    return (
        <JDbox mr>
            ...
            {bi.countDetails?.map((cd) => (
                <JDbox mr key={cd.key}>
                    {autoComma(cd.count)} {unit || "개"} {cd.label}{" "}
                    {autoComma(cd.price)}{" "}
                </JDbox>
            ))}
        </JDbox>
