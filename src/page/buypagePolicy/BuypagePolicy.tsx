...
import {
    useBuypagePolicyDelete,
    useBuypagePolicyUpdate,
} from "../../hook/useBuypage";
import { Fpolicy, me_Me_stores_buypage_policies } from "../../type/api";
import { completeMsg } from "../../utils/onCompletedMessage";
import { promptConfirm } from "../../utils/prompt";
import { PolicyAutoCreateModal } from "./components/PolicyAutoCreateModal";

interface IProp {}

export const BuypagePolicy: React.FC<IProp> = () => {
    const policyAutoSetModalHook = useModal();
    ...
    const handleDelete = (policy: me_Me_stores_buypage_policies) => () => {
        promptConfirm(
            policy.name,
            `아이템을 삭제하실려면 ${policy.name}을 정확하게 입력 해주세요.`,
            () => {
                policyDelete({
                    variables: {
    ...
    return (
        <div>
            <JDpageHeader
                title="예약 폴리시 관리"
                desc="예약페이지에서 동의받을 폴리시를 설정 해주세요."
            />
            <JDcontainer verticalPadding>
                <PolicyAutoCreateModal modalHook={policyAutoSetModalHook} />
                {isEmpty(policies) && (
                    <DotButton onClick={policyAutoSetModalHook.openModal}>
                        정책 자동 설정
                    </DotButton>
                )}
                {!isEmpty(policies) && !editMode && (
                    <div>
                        {policies.map((policy) => (
                            <FoldCard
                                mb
                                ...
                                title={
                                    <Flex>
                                        {policy.name}{" "}
                                        <JDlabel require={policy.require} mr />{" "}
                                        <JDbadge size="small" thema="grey2">
                        ...
                        <JDcard
                            foot={
                                <div>
                                    ...
                                    >
                                        {createMode ? "생성하기" : "수정하기"}
                                    </CardBtn>
                                    {selectPolicy && (
                                        <CardBtn
                                            onClick={handleDelete(selectPolicy)}
                                        >
                                            삭제하기
                                        </CardBtn>
                                    )}
