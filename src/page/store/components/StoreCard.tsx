import { Flex, JDbutton, JDcard, JDicon, JDtypho, Tiny } from '@janda-com/front';
...
export const StoreCard: React.FC<IProp> = ({
    ...
    return <JDcard badges={isCurrentStore ? [{ label: "현재선택", mode: "folded", thema: "point" }] : undefined} selected={selectedStore?._id === store._id} foot={<div>
        <Tiny color={noItem ? "warn" : undefined} mb> <JDicon mr="tiny" icon="info" />{store.itemCount === 0 ? "생성된 상품이 없습니다. 상품을 생성해주세요." : `누적 상품수 ${store.itemCount}`}</Tiny>
        <CardBtn hide={isCurrentStore} mr onClick={handleSelect} thema="black">선택하기</CardBtn>
        <CardBtn mr onClick={handleEdit} thema="primary">수정하기</CardBtn>
        <CardBtn mr onClick={handleDelete} thema="error">삭제하기</CardBtn>
        {isCurrentStore && <CardBtn mr hide={!store.itemCount} onClick={handleList} label={"상품보기"} />}
        ...
        {...props}>
        <Flex vCenter>
            <Info mr="huge" mb label={"타입"} value={storeTypeKr(store.type)} />
            <Info mb label={"생성일"} value={yyyymmdd(store.createdAt)} />
        </Flex>
        <Flex vCenter>
            <Info mr="huge" mb label={"스토어코드"} value={store.code} />
            <Info mb label={"아이템수"} value={store.itemCount.toString()} />
        </Flex>
        <JDtypho size="small">
            {store.description}
        </JDtypho>
