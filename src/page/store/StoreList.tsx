...
import { storeList_StoreList_items, _StoreSort } from "../../type/api";
...
export const Store: React.FC<IProp> = () => {
    ...
    const handleDelete = (store: storeList_StoreList_items) => () => {
        promptConfirm(
            store.name,
            `스토어를 삭제하실려면 ${store.name}을 정확하게 입력 해주세요.`,
            () => {
                deleteMu({
                    variables: {
                        storeId: store._id,
    ...
    return (
        <div>
            ...
            <JDcontainer verticalPadding>
                ...
                <DotButton onClick={storeModalHook.openModal as () => void}>
                    추가하기
                </DotButton>
            </JDcontainer>
            <StoreModal
                ...
                key={storeModalHook.info?.store?._id || "createModal"}
                modalHook={storeModalHook}
            />
        </div>
    );
};
