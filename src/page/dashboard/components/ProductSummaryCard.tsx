    ...
    const Nodes = items.map((item) => [
        <span className="summaryCard__cell">
            {yyyymmddHHmmRangeLabel(
                item.dateRangeForUse?.from,
                item.dateRangeForUse?.to
            )}
        </span>,
        <span className="summaryCard__cell">
            생성 {yyyymmdd(item.createdAt)}
        </span>,
        <span className="summaryCard__cell">{autoComma(item.price)}</span>,
    ]);

    return (
        <SummaryCard
            contentsId="ProductSummaryCard"
            mb
