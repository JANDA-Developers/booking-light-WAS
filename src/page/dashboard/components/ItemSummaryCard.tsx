...
export const ItemSummaryCard: React.FC<IProp> = () => {
    ...
    const Nodes = items.map((item) => [
        <span
            ...
            onClick={handleTitelClick(item._id)}
        >
            {item.name}
        </span>,
        <span className="summaryCard__cell">{yyyymmdd(item.createdAt)}</span>,
        <Clip className="summaryCard__cell">{item.price}</Clip>,
    ]);

    return (
        <SummaryCard
            Empty={<DotButton onClick={handleViewMore}>생성하기</DotButton>}
            contentsId="ItemSummaryCard"
            mb
