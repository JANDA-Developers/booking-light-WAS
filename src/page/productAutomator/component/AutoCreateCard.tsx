...
export const AutoCreateCard: React.FC<IProp> = ({
    autoCreater,
    onToggleDisable,
    ...props
}) => {
            ...
            <div>
                ...
                <Flex mb="small">
                    <Info
                        mr
                        label="마지막 취소일"
                        value={latestDestroy || "취소없음"}
                    />
                    <Info
                        label="제외요일"
                        value={exceptedDayOfWeeks
                            .map((week) => WeekKr[week])
                            .join(", ")}
                    />
                </Flex>
                <Small mb>{description}</Small>
                <DetailBtn
                    mb
                    open={isOpen}
                    onClick={() => {
                ...
                />
                {isOpen && <ProductTemplateTable products={templates} />}
            </div>
        </JDcard>
    );
};
