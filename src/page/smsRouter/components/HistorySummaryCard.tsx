...
export const HistorySummaryCard: React.FC<IProp> = ({ ...props }) => {
    ...
    const Nodes = items.map(item => ([<span className="summaryCard__cell" >{item.receivers.map(receiver => autoHypen(receiver)).join("|")}</span>, <span className="summaryCard__cell">{MMDDhhmm(item.createdAt)}</span>, <span className="summaryCard__cell">{item.pointConsumed}(Point)</span>]));

    return <SummaryCard contentsId="HistorySummaryCard" mb className="summaryCard" head={<Flex between ><span>발신 히스토리</span> <CardHeadButton onClick={handleViewMore} label="자세히보기" /></Flex>} mr contents={Nodes} {...props} />;
};
