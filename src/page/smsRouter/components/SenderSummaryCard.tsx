...
export const SenderSummaryCard: React.FC<IProp> = () => {
    ...
    const Nodes = manager.senders.map((item) => [
        <span className="summaryCard__cell">{item.sender}</span>,
        <span className="summaryCard__cell">
            {notificationMethodKr(item.type)}
        </span>,
        <span className="summaryCard__cell">
            {isRegisteredToAligoKr(item.isRegisteredToAligo || false)}
        </span>,
    ]);

    return (
        <SummaryCard
            contentsId="SenderSummaryCard"
            mb
